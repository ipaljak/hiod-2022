...
void solve_up(bool first, int node, int dad) {
  ...
  for (int nxt : v[node]) {
    ...
    if (first) {
      dp[first][edge_id] = n + 1;
      if (dad != -1) dp[first][edge_id] = min(dp[first][edge_id], 1 + dp[!first][eid[{node, dad}]]);
      if (i > 0) dp[first][edge_id] = min(dp[first][edge_id], pref[i - 1]);
      if (i < deg - 1) dp[first][edge_id] = min(dp[first][edge_id], suff[i + 1]);
    } else {
      dp[first][edge_id] = 0;
      if (dad != -1) dp[first][edge_id] = max(dp[first][edge_id], 1 + dp[!first][eid[{node, dad}]]);
...
int main(void) {
  ...
  for (int i = 0; i < n; ++i)
    if ((int)v[i].size() != 1) root = i;

  solve_down(true, root, -1);
  solve_down(false, root, -1);

  solve_up(true, root, -1);
  solve_up(false, root, -1);
